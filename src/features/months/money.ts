import { parseMoneyToCents, type MoneyLocale } from '../../domain/money.ts'

/** Parses the ARS display format while preserving a leading sign. */
export function parseSignedMoneyToCents(value: unknown, locale: MoneyLocale = 'es-AR'): number | null {
  ...
  const cents = parseMoneyToCents(unsigned, locale)
