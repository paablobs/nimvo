...
import { formatCurrency, parseMoneyToCents, sumCents } from '../../domain/money.ts'
...
export default function ExpensesPanel({ monthId, year, month, expenses, categories, currency = DEFAULT_CURRENCY, onRefresh, onCategoriesRefresh }: Props) {
  ...
  function startEdit(expense: Expense) {
    ...
    setDraft({ spentOn: expense.spentOn, categoryId: expense.categoryId, amount: formatCurrency(expense.amountCents, currency, { symbol: false, locale: numberFormat }), description: expense.description ?? '' })
    setNewCategory(false)
    setError('')
  }
  ...
  return <section className="panel expenses-panel" aria-labelledby="expenses-title">
    ...
    {draft && <form className="inline-form expense-form" onSubmit={submit} noValidate>
      ...
      <label htmlFor="expense-amount">{t('amountCurrency', { currency })} <ArsMoneyInput locale={numberFormat} id="expense-amount" value={draft.amount} onChange={(event) => setDraft((current) => current ? { ...current, amount: event.target.value } : current)} disabled={busy} /></label>
